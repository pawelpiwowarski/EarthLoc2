...
class TestDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # Remove the far away ones if the threhhold is not infinte then we compute all of the 
        # descriptors avaliable 
     
        db_paths = filter_images_far_from_POI(db_paths, point_of_interest, thresh_db)
        db_paths = [Path(p) for p in db_paths]
        logging.info(f"There are candidate {len(db_paths)} images from 2021 to be encoded ")
